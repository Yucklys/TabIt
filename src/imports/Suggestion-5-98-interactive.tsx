...
function FreeForm({ selectedMode = "smart", onModeChange, onCustomize }: {
  selectedMode?: string;
...
function DefaultPageAiSuggestedGroup({ selectedMode, onModeChange, onCustomize }: {
  selectedMode?: string;
  onModeChange?: (mode: string) => void;
  onCustomize?: () => void;
}) {
  return (
    <div className="absolute bg-white content-stretch flex gap-[10px] items-center left-0 rounded-[16px] top-0" data-name="Default Page-AI Suggested Group">
      <FreeForm selectedMode={selectedMode} onModeChange={onModeChange} onCustomize={onCustomize} />
    </div>
  );
}

export default function SuggestionFinal({ selectedMode = "smart", onModeChange, onCustomize }: {
  selectedMode?: string;
  onModeChange?: (mode: string) => void;
  onCustomize?: () => void;
}) {
  return (
    <div className="relative size-full" data-name="suggestion">
      <DefaultPageAiSuggestedGroup selectedMode={selectedMode} onModeChange={onModeChange} onCustomize={onCustomize} />
    </div>
  );
}
